...
public class BlockBlast {
     ...
     * 
     * gridSize: The size of the blockblast grid (ex. inputting 4 would create a
     * size 4x4 grid)
     */
    ...
    public static void main(String[] args) {
        ...
        while (gameActive) {
            ...
            // // TESTING PRINT STATEMENT FOR CHOOSING NEW BLOCK. YOU MUST COMMENT OUT UPON
            // SUBMISSION
            ...
            for (int i = 0; i < gameGrid.length && !placed; i++) {
                for (int j = 0; j < gameGrid[i].length && !placed; j++) {

                    if (i + currentBlock.length <= gameGrid.length &&
                            j + currentBlock[0].length <= gameGrid[0].length) {

                        boolean Place = true;

                        for (int r = 0; r < currentBlock.length; r++) {
                            for (int c = 0; c < currentBlock[r].length; c++) {
                                if (currentBlock[r][c] && gameGrid[i + r][j + c]) {
                                    Place = false;
                                    break;
                                }
                            }
            ...
            boolean[] fullRows = new boolean[gameGrid.length];
            boolean[] fullCols = new boolean[gameGrid[0].length];
            int clearedLines = 0;
